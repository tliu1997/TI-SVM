...
class TISVM(object):
    ...
    def ti_kernel(self, x, y):
        ...
        """
        y_reshape = y.reshape((-1, 64, 64))

        # choose the maximum kernel for each pair of sample
        kernel_final = np.zeros((x.shape[0], y.shape[0]))

        for k in range(-int(y_reshape.shape[1] / 2), int(y_reshape.shape[1] / 2), 4):
            for l in range(-int(y_reshape.shape[2] / 2), int(y_reshape.shape[2] / 2), 4):
                # shift
...
# SVM with locality and translational-invariant kernel
class LTISVM(object):
    ...
    def locality_kernel(self, x, y):
        X1 = np.transpose(x.reshape((-1, 64, 64, 1)), (0, 3, 1, 2))
        Y1 = np.transpose(y.reshape((-1, 64, 64, 1)), (0, 3, 1, 2))
